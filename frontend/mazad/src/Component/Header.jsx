...
useEffect(() => {
    ...
    const category = sessionStorage.getItem('category') || 'sedan';
    setSelectedCategory(category);

    if (!token) {
        sessionStorage.setItem('category', 'sedan'); 
...
const handleAuthAction = () => {
  ...
  return (
    <header className="bg-gradient-to-r bg-[#758e9d] text-white shadow-lg sticky top-0 z-[1000]">
      <div className="container mx-auto px-4 py-3">
        <div className="flex justify-between items-center">
          <Link to="/sedan" className="flex items-center space-x-2">
...
<select 
  className="bg-white text-blue-800 px-3 py-2 rounded-md" 

...
const MyCars = () => {
  ...
  useEffect(() => {
    const fetchsportsCars = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/car-listings/category/sports');
        const sportsCars = response.data.map(car => {
          return {
            id: car._id,
            type: car.category,
        ...
        setItems(sportsCars);
        const initialImageIndexes = sportsCars.reduce((acc, car) => {
          acc[car.id] = 0;
          return acc;
        }, {});
        ...
        console.error('Error fetching sports cars:', error.response ? error.response.data : error.message);
      }
    };
  ...
  return (
    <main className="main">
      ...
      <h1 className="mb-12 text-4xl font-bold text-primary text-center"> سيارات رياضية</h1>
